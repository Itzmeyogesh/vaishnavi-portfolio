...
const About = () => (
  <section
    id="about"
    className="relative min-h-screen py-24 px-4 sm:px-8 lg:px-14 text-white flex items-center justify-center overflow-hidden"
  >
    {/* 🔷 Improved Background */}
    ...
    <div className="max-w-6xl w-full text-center space-y-12">
      ...
      <motion.div
        ...
        className="bg-white/5 backdrop-blur-xl px-8 sm:px-12 py-10 sm:py-14 rounded-3xl shadow-2xl space-y-6 border border-white/10"
      >
        <h2 className="text-4xl sm:text-5xl font-bold text-cyan-400">About Me</h2>
      ...
      {/* 🔹 Skill Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 gap-y-10 px-2 sm:px-4">
        {skills.map((item, i) => (
          <motion.div
            key={i}
          ...
          >
            ...
            <p className="text-gray-300 text-sm">{item.tools}</p>
          </motion.div>
        ))}
      </div>
    </div>
